...
interface TreeNodeRowProps {
  ...
  shareMode?: boolean; // Флаг для режима публичного просмотра
  getShareUrl?: (id: number) => Promise<string>; // Функция для получения share-ссылки
}

function TreeNodeRow({ node, depth, onCreateChild, onArchive, onOpen, shareMode, getShareUrl }: TreeNodeRowProps) {
  const [expanded, setExpanded] = useState(true);
  const hasChildren = node.children.length > 0;

  const handleOpen = async () => {
    if (shareMode && getShareUrl) {
      // В share-режиме получаем публичную ссылку и переходим на неё
      try {
      ...
      >
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="flex min-w-0 items-center gap-1.5">
            ...
            {/* Кнопка создания вложенной страницы скрыта в share-режиме */}
            {!shareMode && (
              <button
                type="button"
        ...
        {/* Col 3: actions - скрыты в share-режиме */}
        {!shareMode && (
...
interface KnowledgeBaseTabProps {
  shareMode?: boolean;
  getShareUrl?: (id: number) => Promise<string>;
}

export function KnowledgeBaseTab({ shareMode = false, getShareUrl }: KnowledgeBaseTabProps = {}) {
      ...
      {!isLoading && !error && (
        <div className="rounded-lg border bg-white">
          ...
          <div
            ...
            style={{ gridTemplateColumns: shareMode ? '1fr max-content' : '1fr max-content max-content' }}
          >
            <span>Название</span>
            <span>Обновлён</span>
            {!shareMode && <span className="sr-only">Действия</span>}
          </div>
