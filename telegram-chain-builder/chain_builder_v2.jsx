...
function useAutoSave(value, onSave, delay = 500) {
...
function NodeCard({ node, isStart, isSelected, onMouseDown, onClick, onContextMenu }) {
  ...
  return (
    <div
      ...
      style={{
        ...{ left: node.pos_x, top: node.pos_y, width: NODE_W, height: NODE_H },
...
export default function ChainBuilder() {
  ...
  useAutoSave(state, async (s) => {
    if (!s.dirty) return;
    setSaving(true);
    setError(null);
    try {
      await mockApi.saveGraph(s);
      dispatch({ type: "SAVED" });
    } catch (err) {
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-600">Загрузка...</div>;
