...
export function ClientsSectionShell({
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-3">
      <div
        className={cn(
          '-mx-4 sticky top-0 z-20 border-b border-black/5 bg-[#f4f1ea]/95 px-4 pb-3 pt-2 backdrop-blur',
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h1 className="text-xl font-semibold tracking-tight text-slate-950 sm:text-2xl lg:text-3xl">{title}</h1>
            <p className="mt-0.5 hidden max-w-3xl text-sm text-slate-600 sm:block">{description}</p>
          </div>
        ...
        <nav
          ...
          className="-mx-1 mt-2 flex gap-1.5 overflow-x-auto px-1 pb-1 sm:gap-2 hide-scrollbar"
          ...
          {CLIENTS_SECTION_LINKS.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  'shrink-0 rounded-full border px-3 py-2 text-sm font-medium transition-colors sm:py-1.5',
                  isActive
