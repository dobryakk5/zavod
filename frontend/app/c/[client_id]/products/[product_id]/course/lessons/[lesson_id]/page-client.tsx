...
}: PublicProductCourseLessonPageClientProps = {}) {
  const { client_id: rawClientId, product_id: rawProductId, lesson_id: rawLessonId } =
    useParams<Partial<RouteParams> & { product_id: string; lesson_id: string }>();
  const router = useRouter();
  const searchParams = useSearchParams();
  const clientId = resolvedClientId ?? Number(rawClientId);
  const productId = Number(rawProductId);
  const lessonId = Number(rawLessonId);
  const coursePath = useCustomDomainPaths
    ? `/products/${productId}/course`
    : `/c/${clientId}/products/${productId}/course`;
    ...
    return (
      <div className="mx-auto max-w-4xl space-y-3 p-6">
        ...
        <div className="flex gap-2">
          <Button type="button" variant="outline" onClick={() => router.push(coursePath)}>
            К курсу
          </Button>
