...
export default function PublicCoachingStepPage({ clientId, stepId }: PublicCoachingStepPageProps) {
  ...
  if (resolvedContactId) {
    backQuery.set('contact_id', String(resolvedContactId));
  }
  const backPath = source === 'tasks'
    ? `/c/${clientId}/tasks${backQuery.toString() ? `?${backQuery.toString()}` : ''}`
    : `/c/${clientId}/coaching${backQuery.toString() ? `?${backQuery.toString()}` : ''}`;
  ...
  const loadStep = useCallback(async () => {
    ...
    try {
      const [portalResult, historyResult] = await Promise.allSettled([
        coachingApi.getClientCoachingPortal(clientId, { contactId: resolvedContactId }),
        coachingApi.getClientStepHistory(clientId, stepId, { contactId: resolvedContactId }),
      ]);
      ...
      const matched = findGoalStep(portal, stepId);

      if (!matched.goal || !matched.step) {
        setState({
          clientName: String(portal.client?.name || ''),
          goal: null,
          step: null,
          history,
        });
  ...
  }, [clientId, resolvedContactId, stepId, syncDrafts]);
  ...
  const replaceStep = useCallback((nextStep: CoachGoalStep) => {
    setState((current) => {
      if (!current.goal) {
      ...
      return {
        ...
        goal: {
          ...current.goal,
          steps: current.goal.steps.map((item) => (String(item.id) === String(nextStep.id) ? nextStep : item)),
        },
        step: nextStep,
      };
    });
  ...
  const refreshHistory = useCallback(async () => {
    const history = await coachingApi.getClientStepHistory(clientId, stepId, { contactId: resolvedContactId });
    setState((current) => ({ ...current, history }));
  }, [clientId, resolvedContactId, stepId]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await coachingApi.editClientCoachingStep(
        clientId,
        step.id,
        {
          text: nextTitle,
  ...
  const handleAddComment = async () => {
    ...
    try {
      const entry = await coachingApi.addClientStepComment(clientId, step.id, note, { contactId: resolvedContactId });
    ...
    return (
      <div className="min-h-screen bg-[#f5f4f0] p-5">
        <div className="mx-auto max-w-[960px] space-y-4">
          <Link href={backPath} className="text-[13px] text-[#185fa5] hover:underline">
            Вернуться назад
          </Link>
      ...
      <div className="mx-auto max-w-[1100px] space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-1">
            ...
            <div className="text-[24px] font-medium leading-tight">{state.clientName || 'Клиент'}</div>
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-3 rounded-[8px] border border-[#e0ddd6] bg-[#f8f6f1] px-3 py-3 text-[14px] text-[#4f4b45]">
                <input
                  ...
                  aria-label="Отмечать этот шаг как веху"
...
function findGoalStep(portal: PublicClientCoachingPortalResponse, stepId: string) {
  const goals = Array.isArray(portal.goals) ? portal.goals : [];
