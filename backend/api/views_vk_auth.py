...
class VkBridgeAuthView(APIView):
    """Exchange VK Bridge access_token for local JWT cookies (Mini App flow)."""
    ...
    def post(self, request):
        ...
        if not access_token or not vk_user_id:
            return Response({"error": "Missing access_token or user_id"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        profile = _fetch_vk_profile(access_token=access_token, user_id=vk_user_id) or {}
        if not profile:
            return Response({"error": "Failed to verify VK token"}, status=status.HTTP_400_BAD_REQUEST)

        if str(profile.get("id")) != str(vk_user_id):
            return Response({"error": "Token user_id mismatch"}, status=status.HTTP_400_BAD_REQUEST)

        user = _find_user_by_vk_provider_id(vk_user_id)
        if user is None:
            user = _get_or_create_user_and_client(profile=profile, vk_user_id=vk_user_id, email=None)

        linked, error = _link_vk_to_user(user=user, vk_user_id=vk_user_id, profile=profile, email=None)
        if not linked:
            return Response({"error": error}, status=status.HTTP_409_CONFLICT)
