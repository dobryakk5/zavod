...
def add_semantic_clusters_prompt(apps, schema_editor):
...
def remove_semantic_clusters_prompt(apps, schema_editor):
    GeneratorPrompt = apps.get_model("core", "GeneratorPrompt")
    GeneratorPrompt.objects.filter(code="semantic_clusters_from_group").delete()


class Migration(migrations.Migration):

    dependencies = [
        ("core", "0118_semantic_groups_prompt"),
    ]

    operations = [
        migrations.RunPython(add_semantic_clusters_prompt, reverse_code=remove_semantic_clusters_prompt),
    ]
