...
This document describes all available API endpoints in the Zavod project, including both documented and newly implemented functionality.
...
3. [Content Management](#content-management)
   ...
   - [Stories](#stories)
   - [Templates](#templates)
   - [Post Types & Tones](#post-types--tones)
4. [Scheduling & Publishing](#scheduling--publishing)
...
```json
{
  "client": {
    ...
    "name": "Client Name",
    "slug": "client-slug",
    "last_image_generation_at": "2024-01-01T00:00:00Z",
    "last_video_generation_at": "2024-01-01T00:00:00Z"
  },
  "role": "owner"
}
```
...
### GET /api/client/generation-events/

Get generation event summary and limits.
...
### POST /api/client/expert-books/

Generate AI-powered expert book recommendations.

**Request Body:**
```json
{
  "pains": "Боли аудитории",
  "desires": "Желания аудитории",
  "avatar": "Портрет целевой аудитории",
  "language": "ru"
}
```
...
{
  "success": true,
  "text": "Рекомендованные книги:\\n1. Книга 1 - Автор 1: Причина 1\\n2. Книга 2 - Автор 2: Причина 2",
  "books": [
    {
      "title": "Книга 1",
      "author": "Автор 1",
...
**Supported models:** `openrouter`, `veo_photo`, `giga_photo`, `nanobanana`, `pollinations`, `huggingface`, `flux2`, `sora_images`, `telegram_bot`, `veo`, `giga`
...
{
  "success": false,
  "error": "Генерация изображения будет доступна через 5 мин 30 сек",
  "cooldown_seconds": 330,
  "cooldown_ends_at": "2024-01-01T01:05:30Z",
  "cooldown_type": "image"
}
```
...
**Supported methods:** `wan`, `veo`
**Supported sources:** `image`, `text`
...
#### POST /api/posts/plan-weekly/

Plan weekly posts generation.
...
{
  "success": true,
  "message": "Запущена генерация 7 постов по шаблону «Template Name»",
  "task_id": "task_123"
}
```
...
#### DELETE /api/posts/{id}/delete_image/
...
#### DELETE /api/posts/{id}/delete_video/

Delete a specific video from the post.

**Query Parameters:**
- `video_id` (required): Video ID to delete
...
### GET /rss/{client_slug}.xml

Public RSS feed for Yandex Zen consumption.

**Response:** XML RSS feed with recent posts

### GET /api/posts-list/

Legacy posts list view (backward compatibility).
...
- `"Генерация {kind} будет доступна через {time}"` - Media generation cooldown
- `"Unknown {model/method/source} "{value}""` - Invalid parameter value
...
- **Media generation**: Owner or Editor only (with cooldown limits)
- **Channel analysis**: Owner or Editor only (with generation limits)

### Rate Limiting:

- **Image generation**: 1 hour cooldown between generations
- **Video generation**: 1 hour cooldown between generations
- **Content generation**: Based on client plan limits
- **Channel analysis**: Based on client plan limits
...
## Supported Media Generation Models

- **Image models**: `openrouter`, `veo_photo`, `giga_photo`
- **Video methods**: `wan`, `veo`
- **Video sources**: `image`, `text`

## Implementation Notes

1. **Asynchronous Processing**: Most generation operations run asynchronously via Celery tasks
2. **Rate Limiting**: Media generation has cooldown periods to prevent abuse
3. **Generation Limits**: Content generation is limited based on client plan
4. **Error Handling**: Comprehensive error handling with clear error messages
5. **Authentication**: JWT-based authentication with cookie support
